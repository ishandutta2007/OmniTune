# Copyright 2025 HuggingFace Inc., the KVCache.AI team, Approaching AI, and the OmniTune team.
#
...
class KTransformersArguments:
    ...
    def _normalize_advanced_kt_config(self, raw_config: Any) -> dict[str, Any]:
        ...
        if not isinstance(raw_config, dict):
            raise TypeError("OmniTune `kt_config` must be a flat mapping.")
        ...
        if conflicts:
            raise ValueError(f"These `kt_config` values are derived from OmniTune arguments: {conflicts}.")
        return config
    ...
    def configure_kt_checkpointing(self, training_args: Any) -> None:
        ...
        if getattr(training_args, "gradient_checkpointing", False):
            raise ValueError(
                "KTransformers uses OmniTune's `disable_gradient_checkpointing`; "
    ...
    def get_kt_config_dict(
        ...
        r"""Map OmniTune-owned training values to the public KT configuration."""
    ...
    def apply_kt_config(self, finetuning_args: Any, training_args: Any, model_max_length: int | None) -> None:
        r"""Apply OmniTune KT args to transformers/accelerate KT integration points."""
        if not self.use_kt:
            return
