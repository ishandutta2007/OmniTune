...
class FSDPTurboParallelState:
    """Own FSDPTurbo's expert topology without extending OmniTune's global interface."""
...
class FSDPTurboFSDP2Engine(FSDP2Engine):
    """FSDPTurbo EP adapter that reuses OmniTune's init/load flow.

    Design:
    - FSDPTurbo owns EP / EFSDP only.
    - OmniTune owns PEFT / FSDP / CP / init-load / checkpoint lifecycle.
    """
    ...
    def prepare_model(self, model: HFModel) -> HFModel:
        # Apply FSDPTurbo EP first, then shard the remaining parameters with OmniTune FSDP2.
