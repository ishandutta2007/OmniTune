...
def launch():
    ...
    # NOTE:
    # `omnitune-cli <command> ...` enters here first.
    ...
    if command in _DIST_TRAIN_COMMANDS and (
        ...
        from omnitune.v1.plugins.model_plugins.peft import merge_and_export_model
        ...
        if command in ("train", "sft"):
            from omnitune.v1.trainers.sft_trainer import run_sft

            run_sft()
        elif command == "dpo":
            from omnitune.v1.trainers.dpo_trainer import run_dpo

            run_dpo()
        elif command == "rm":
            from omnitune.v1.trainers.rm_trainer import run_rm
