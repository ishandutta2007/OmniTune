...
def build_chat_template_with_generation(
    ...
    Prefers the OmniTune registered template (same formatting as HF Trainer),
    ...
    if template_name:
        try:
            ...
            logger.warning_rank0(f"Failed to build OmniTune chat template '{template_name}': {exc}")
...
def _example_to_record(
    ...
    r"""Convert an aligned OmniTune example to Megatron Bridge JSONL format."""
...
def export_dataset_for_megatron_bridge(
    ...
    r"""Export aligned OmniTune datasets to Megatron Bridge JSONL files."""
